...
def process_run(run, x_test, y_test, train_dataset, type):
  if '%' in run:
...
if __name__ == '__main__':
  ...
  for run in runs:
    if 'control' in run:
      type = 'mean' # this is ignored anyway
      threshold, n0, n1, ntr, nte, f0, t0, f1, t1, f2tr, t2tr, f2te, t2te, f2te0, f2te1 = process_run(run, x_test, y_test, train_dataset, type)
      df.loc[len(df.index)] = (threshold, n0, n1, ntr, nte, f0, t0, f1, t1, f2tr, t2tr, f2te, t2te, f2te0, f2te1,type)
    else:
      for type in ('mean', 'min'):#, 'max', 'ood'):
        threshold, n0, n1, ntr, nte, f0, t0, f1, t1, f2tr, t2tr, f2te, t2te, f2te0, f2te1 = process_run(run, x_test, y_test, train_dataset, type)
        df.loc[len(df.index)] = (threshold, n0, n1, ntr, nte, f0, t0, f1, t1, f2tr, t2tr, f2te, t2te, f2te0, f2te1,type)
  df.to_csv(sys.argv[1]+testval+'dfn1.csv', index=False)
