...
def run(id_classes, train_ood_classes):
  ...
  X_train, Y_train, test_ood_classes = ood_utils.ood_use_classes_balanced(id_classes, train_ood_classes)
...
if __name__ == '__main__':
  ...
  for i in range(10):
    ...
    mod.save('oodrand_' + name + '%' +str(time.time()-1675389656))
    print(len(dat))
    thres.call_thres(threshold, dat, y_test)
    num_images(threshold, np.reshape(dat[10:], (10000, 3)))
