...
def run(id_classes, train_ood_classes):
  ...
  X_train, Y_train = med_utils.med_use_classes_control(id_classes, train_dataset)
...
if __name__ == '__main__':
  ...
  for i in range(10):
    ...
    mod.save('medcontrol_' + name + '%' +str(time.time()-1675389656))
    print(len(dat))
    thres.call_thres_control(threshold, dat, y_val)
    num_images(threshold, np.reshape(dat[9:], (10004, 2)))
